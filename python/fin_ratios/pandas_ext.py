...
_ALIASES: dict[str, list[str]] = {
    ...
    "operating_cash_flow":   ["operating_cash_flow", "cash_from_operations", "cfo",
                               "net_cash_from_operating_activities"],
    "capex":                 ["capex", "capital_expenditures", "purchase_of_ppe",
                               "capital_expenditure"],
    "total_assets":          ["total_assets", "assets"],
    "total_equity":          ["total_equity", "shareholders_equity", "total_stockholders_equity",
                               "equity", "book_value"],
...
_ALL_METRICS = [
    "pe", "pb", "ps", "peg", "gross_margin", "operating_margin", "net_margin",
    "ebitda_margin", "roe", "roa", "roic", "roce", "fcf_margin", "fcf_conversion",
    "ocf_to_sales", "capex_to_revenue", "current_ratio", "quick_ratio",
    "debt_to_equity", "net_debt_to_equity", "net_debt_to_ebitda",
    "debt_to_assets", "interest_coverage", "asset_turnover",
    "receivables_turnover", "inventory_turnover", "dso", "dio", "dpo",
    "ev_ebitda", "tobin_q", "free_cash_flow", "fcf_yield",
]
...
def _compute_row(row_dict: dict[str, float], metrics: list[str]) -> dict[str, Optional[float]]:
    ...
    ev = g("enterprise_value") or (
        (g("market_cap", 0) + g("total_debt", 0) - g("cash", 0))
        if g("market_cap") else None
    )
    ...
    _dispatch: dict[str, Any] = {
        ...
        "peg":             lambda: _fr.peg(pe_ratio=_fr.pe(market_cap=g("market_cap", 0), net_income=g("net_income", 0)), eps_growth_rate_pct=g("eps_growth_pct", 0)) if g("eps_growth_pct") and g("market_cap") else None,
        "ev_ebitda":       lambda: _fr.ev_ebitda(ev=ev, ebitda=g("ebitda", 0)) if ev and g("ebitda") else None,
        "gross_margin":    lambda: _fr.gross_margin(gross_profit=g("gross_profit", 0), revenue=g("revenue", 0)),
        "operating_margin":lambda: _fr.operating_margin(ebit=g("ebit", 0), revenue=g("revenue", 0)),
        "net_margin":      lambda: _fr.net_profit_margin(net_income=g("net_income", 0), revenue=g("revenue", 0)),
        "ebitda_margin":   lambda: _fr.ebitda_margin(ebitda=g("ebitda", 0), revenue=g("revenue", 0)) if g("ebitda") else None,
        "roe":             lambda: _fr.roe(net_income=g("net_income", 0), avg_total_equity=g("total_equity", 0)),
        "roa":             lambda: _fr.roa(net_income=g("net_income", 0), avg_total_assets=g("total_assets", 0)),
        "roic":            lambda: _fr.roic(nopat_value=nopat_val, invested_capital=ic_val) if nopat_val and ic_val else None,
        "roce":            lambda: _fr.roce(ebit=g("ebit", 0), total_assets=g("total_assets", 0), current_liabilities=g("current_liabilities", 0)),
        "fcf_margin":      lambda: _fr.fcf_margin(fcf=fcf, revenue=g("revenue", 0)) if fcf else None,
        "fcf_conversion":  lambda: _fr.fcf_conversion(fcf=fcf, net_income=g("net_income", 0)) if fcf else None,
        "ocf_to_sales":    lambda: _fr.ocf_to_sales(operating_cash_flow=g("operating_cash_flow", 0), revenue=g("revenue", 0)),
        "capex_to_revenue":lambda: _fr.capex_to_revenue(capex=g("capex", 0), revenue=g("revenue", 0)),
        "free_cash_flow":  lambda: fcf,
        "fcf_yield":       lambda: _fr.fcf_yield(fcf=fcf, market_cap=g("market_cap", 0)) if fcf and g("market_cap") else None,
        "current_ratio":   lambda: _fr.current_ratio(current_assets=g("current_assets", 0), current_liabilities=g("current_liabilities", 0)),
        "quick_ratio":     lambda: _fr.quick_ratio(cash=g("cash", 0), short_term_investments=0, accounts_receivable=g("accounts_receivable", 0), current_liabilities=g("current_liabilities", 0)),
        "dso":             lambda: _fr.dso(accounts_receivable=g("accounts_receivable", 0), revenue=g("revenue", 0)),
        "dio":             lambda: _fr.dio(inventory=g("inventory", 0), cogs=g("cogs", 0)) if g("cogs") else None,
        "dpo":             lambda: _fr.dpo(accounts_payable=g("accounts_payable", 0), cogs=g("cogs", 0)) if g("cogs") else None,
        "debt_to_equity":  lambda: _fr.debt_to_equity(total_debt=g("total_debt", 0), total_equity=g("total_equity", 0)),
        "net_debt_to_equity": lambda: _fr.net_debt_to_equity(total_debt=g("total_debt", 0), cash=g("cash", 0), total_equity=g("total_equity", 0)),
        "net_debt_to_ebitda": lambda: _fr.net_debt_to_ebitda(total_debt=g("total_debt", 0), cash=g("cash", 0), ebitda=g("ebitda", 0)) if g("ebitda") else None,
        "debt_to_assets":  lambda: _fr.debt_to_assets(total_debt=g("total_debt", 0), total_assets=g("total_assets", 0)),
        "interest_coverage": lambda: _fr.interest_coverage_ratio(ebit=g("ebit", 0), interest_expense=g("interest_expense", 0)),
        "asset_turnover":  lambda: _fr.asset_turnover(revenue=g("revenue", 0), avg_total_assets=g("total_assets", 0)),
        "receivables_turnover": lambda: _fr.receivables_turnover(revenue=g("revenue", 0), avg_accounts_receivable=g("accounts_receivable", 0)),
        "inventory_turnover": lambda: _fr.inventory_turnover(cogs=g("cogs", 0), avg_inventory=g("inventory", 0)) if g("cogs") else None,
        "tobin_q":         lambda: _fr.tobin_q(market_cap=g("market_cap", 0), total_debt=g("total_debt", 0), total_assets=g("total_assets", 0)),
    }
...
def _ratios_polars(df: Any, ratios: list[str], groupby: Optional[str], inplace: bool) -> Any:
    ...
    for row in df.iter_rows(named=True):
        row_d = {canonical: row[col] for canonical, col in col_map.items()
