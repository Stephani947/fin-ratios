...
def health_score(
    ...
    if roic is not None:
        if wacc is not None:
            ...
            signals["value_creation"] = f"ROIC {roic*100:.1f}% vs WACC {wacc*100:.1f}% (spread {spread*100:+.1f}%)"
        else:
    ...
    if net_margin is not None:
        if net_margin >= 0.20:     nm_s = 100.0
        elif net_margin >= 0.10:   nm_s = 65.0 + (net_margin - 0.10) / 0.10 * 35.0
        elif net_margin >= 0.05:   nm_s = 35.0 + (net_margin - 0.05) / 0.05 * 30.0
        elif net_margin >= 0.0:    nm_s = net_margin / 0.05 * 35.0
        else:                      nm_s = max(0.0, 35.0 + net_margin * 7)
        prof_scores.append(nm_s)

    if gross_margin is not None:
        if gross_margin >= 0.60:   gm_s = 100.0
        elif gross_margin >= 0.40: gm_s = 65.0 + (gross_margin - 0.40) / 0.20 * 35.0
        elif gross_margin >= 0.20: gm_s = 30.0 + (gross_margin - 0.20) / 0.20 * 35.0
        else:                      gm_s = max(0.0, gross_margin / 0.20 * 30.0)
        prof_scores.append(gm_s)

    if roe is not None:
        if roe >= 0.25:    roe_s = 100.0
        elif roe >= 0.15:  roe_s = 70.0 + (roe - 0.15) / 0.10 * 30.0
        elif roe >= 0.08:  roe_s = 40.0 + (roe - 0.08) / 0.07 * 30.0
        elif roe >= 0.0:   roe_s = roe / 0.08 * 40.0
        else:              roe_s = 0.0
    ...
    if fcf_conversion is not None:
        if fcf_conversion >= 1.0:   fc_s = 100.0
        elif fcf_conversion >= 0.8: fc_s = 75.0 + (fcf_conversion - 0.8) / 0.2 * 25.0
        elif fcf_conversion >= 0.5: fc_s = 40.0 + (fcf_conversion - 0.5) / 0.3 * 35.0
        elif fcf_conversion >= 0.0: fc_s = fcf_conversion / 0.5 * 40.0
        else:                       fc_s = 0.0
        fcf_scores.append(fc_s)
        signals["fcf_quality"] = f"FCF Conversion {fcf_conversion:.2f}x"

    if fcf_margin is not None:
        if fcf_margin >= 0.20:   fm_s = 100.0
        elif fcf_margin >= 0.10: fm_s = 65.0 + (fcf_margin - 0.10) / 0.10 * 35.0
        elif fcf_margin >= 0.05: fm_s = 35.0 + (fcf_margin - 0.05) / 0.05 * 30.0
        elif fcf_margin >= 0.0:  fm_s = fcf_margin / 0.05 * 35.0
        else:                    fm_s = 0.0
    ...
    if revenue_growth is not None:
        if revenue_growth >= 0.20:   gr_s = 100.0
        elif revenue_growth >= 0.10: gr_s = 70.0 + (revenue_growth - 0.10) / 0.10 * 30.0
        elif revenue_growth >= 0.05: gr_s = 45.0 + (revenue_growth - 0.05) / 0.05 * 25.0
        elif revenue_growth >= 0.0:  gr_s = revenue_growth / 0.05 * 45.0
        else:                        gr_s = max(0.0, 30.0 + revenue_growth * 6)
    ...
    if debt_to_equity is not None:
        if debt_to_equity > 3.0:   bs_adjustments -= 5.0
        elif debt_to_equity > 2.0: bs_adjustments -= 2.5
        elif debt_to_equity < 0.5: bs_adjustments += 2.5

    if current_ratio is not None:
        if current_ratio < 1.0:    bs_adjustments -= 5.0
        elif current_ratio < 1.5:  bs_adjustments -= 2.0
        elif current_ratio >= 2.0: bs_adjustments += 2.0

    if interest_coverage is not None:
        if interest_coverage < 1.5:  bs_adjustments -= 8.0
        elif interest_coverage < 3.0: bs_adjustments -= 3.0
        elif interest_coverage > 8.0: bs_adjustments += 3.0
    ...
    if total_weight == 0:
        ...
        raw_score = sum(components[k] * w for k, w in weights.items() if k in components) / total_weight
