...
def compute_all(data: Any, prior: Optional[Any] = None) -> dict[str, Any]:
    ...
    result['peg']         = _fr.peg(pe_ratio=result['pe'], eps_growth_rate_pct=d.eps_growth_pct) if result['pe'] and d.eps_growth_pct else None

    fcf = _fr.free_cash_flow(operating_cash_flow=d.operating_cash_flow, capex=d.capex)
    result['free_cash_flow'] = fcf
    result['p_fcf']       = _fr.p_fcf(market_cap=d.market_cap, operating_cash_flow=d.operating_cash_flow, capex=d.capex) if d.operating_cash_flow else None
    ...
    result['tobin_q']     = _fr.tobin_q(market_cap=d.market_cap, total_debt=d.total_debt, total_assets=d.total_assets)
    result['graham_number'] = _fr.graham_number(
        eps=safe_divide(d.net_income, d.shares_outstanding),
        book_value_per_share=safe_divide(d.total_equity, d.shares_outstanding),
    ) if d.shares_outstanding else None
    ...
    result['ebitda_margin']    = _fr.ebitda_margin(ebitda=d.ebitda, revenue=d.revenue) if d.ebitda else None
    ...
    result['roce']  = _fr.roce(ebit=d.ebit, total_assets=d.total_assets, current_liabilities=d.current_liabilities)
    ...
    result['ocf_to_sales']   = _fr.ocf_to_sales(operating_cash_flow=d.operating_cash_flow, revenue=d.revenue)
    ...
    # ── Liquidity ─────────────────────────────────────────────────────────────
    result['current_ratio'] = _fr.current_ratio(current_assets=d.current_assets, current_liabilities=d.current_liabilities)
    result['quick_ratio']   = _fr.quick_ratio(cash=d.cash, short_term_investments=0, accounts_receivable=d.accounts_receivable, current_liabilities=d.current_liabilities)
    ...
    result['cash_conversion_cycle'] = _fr.cash_conversion_cycle(
        dso_days=dso_v, dio_days=dio_v or 0, dpo_days=dpo_v or 0
    ) if dso_v is not None else None

    # ── Solvency ──────────────────────────────────────────────────────────────
    result['debt_to_equity']      = _fr.debt_to_equity(total_debt=d.total_debt, total_equity=d.total_equity)
    result['net_debt_to_equity']  = _fr.net_debt_to_equity(total_debt=d.total_debt, cash=d.cash, total_equity=d.total_equity)
    result['net_debt_to_ebitda']  = _fr.net_debt_to_ebitda(total_debt=d.total_debt, cash=d.cash, ebitda=d.ebitda) if d.ebitda else None
    result['debt_to_assets']      = _fr.debt_to_assets(total_debt=d.total_debt, total_assets=d.total_assets)
    result['interest_coverage']   = _fr.interest_coverage_ratio(ebit=d.ebit, interest_expense=d.interest_expense)
    result['equity_multiplier']   = _fr.equity_multiplier(total_assets=d.total_assets, total_equity=d.total_equity)

    # ── Efficiency ────────────────────────────────────────────────────────────
    result['asset_turnover']       = _fr.asset_turnover(revenue=d.revenue, avg_total_assets=d.total_assets)
    result['receivables_turnover'] = _fr.receivables_turnover(revenue=d.revenue, avg_accounts_receivable=d.accounts_receivable)
    result['inventory_turnover']   = _fr.inventory_turnover(cogs=d.cogs, avg_inventory=d.inventory) if d.cogs else None
    result['payables_turnover']    = _fr.payables_turnover(cogs=d.cogs, avg_accounts_payable=d.accounts_payable) if d.cogs else None
    ...
    if prior is not None:
        ...
        try:
            result['piotroski'] = _fr.piotroski_f_score(
                current_net_income=d.net_income, current_total_assets=d.total_assets,
                current_operating_cf=d.operating_cash_flow, current_long_term_debt=d.long_term_debt,
                current_current_assets=d.current_assets, current_current_liabilities=d.current_liabilities,
                current_shares_outstanding=d.shares_outstanding, current_gross_profit=d.gross_profit,
                current_revenue=d.revenue,
                prior_net_income=p.net_income, prior_total_assets=p.total_assets,
                prior_long_term_debt=p.long_term_debt, prior_current_assets=p.current_assets,
                prior_current_liabilities=p.current_liabilities,
                prior_shares_outstanding=p.shares_outstanding,
                prior_gross_profit=p.gross_profit, prior_revenue=p.revenue,
            )
        except Exception:
        ...
        try:
            ...
            result['montier_c'] = montier_c_score(
                current_net_income=d.net_income, current_operating_cash_flow=d.operating_cash_flow,
                current_accounts_receivable=d.accounts_receivable, current_revenue=d.revenue,
                current_inventory=d.inventory, current_cogs=d.cogs or d.revenue - d.gross_profit,
                current_cash=d.cash, current_total_assets=d.total_assets,
                current_long_term_debt=d.long_term_debt, current_gross_profit=d.gross_profit,
                prior_accounts_receivable=p.accounts_receivable, prior_revenue=p.revenue,
                prior_inventory=p.inventory, prior_cogs=p.cogs or p.revenue - p.gross_profit,
                prior_cash=p.cash, prior_total_assets=p.total_assets,
                prior_long_term_debt=p.long_term_debt, prior_gross_profit=p.gross_profit,
            )
        except Exception:
    ...
    try:
        ...
        piotroski_int = piotroski_score_val.get('score') if isinstance(piotroski_score_val, dict) else None
