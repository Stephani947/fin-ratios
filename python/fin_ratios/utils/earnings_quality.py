...
_ZERO_FIELDS = frozenset({
    ...
    "interest_expense", "income_tax_expense", "ebt", "ebit",
})
...
_ALIASES: dict[str, list[str]] = {
    ...
    "accounts_receivable": [
        "net_receivables", "receivables", "trade_receivables",
...
def _score_accruals(accs: list[_Acc]) -> tuple[float, list[str]]:
    ...
    for i, d in enumerate(accs):
        ...
        if ni == 0 and cfo == 0:
            continue
        avg_assets = (
            (d.total_assets + accs[i - 1].total_assets) / 2.0
            if i > 0 else d.total_assets
        )
        if avg_assets <= 0:
            continue
    ...
    quality = (
        "cash-backed" if mean_accrual < 0.0 else
        "modest accruals" if mean_accrual < 0.05 else
        "high accruals"
    )
    return score, [
...
def _score_revenue_recognition(accs: list[_Acc]) -> tuple[float, list[str]]:
    ...
    quality = (
        "conservative" if mean_spread > 0.05 else
        "neutral" if mean_spread > -0.05 else
        "aggressive"
    )
    return score, [
...
def _score_gross_margin_stability(accs: list[_Acc]) -> tuple[float, list[str]]:
    ...
    gm_series = [
        ...
        for d in accs
        if d.revenue > 0 and d.gross_profit > 0
    ]
    if len(gm_series) < 2:
...
def _score_asset_efficiency(accs: list[_Acc]) -> tuple[float, list[str]]:
    ...
    turnover = [
        ...
        for d in accs
        if d.total_assets > 0 and d.revenue > 0
    ]
    if len(turnover) < 2:
...
class EarningsQualityScore:
    ...
    def _repr_html_(self) -> str:
        colours = {
            "high": "#1a7f37", "medium": "#0969da",
            "low":  "#9a6700", "poor":   "#cf222e",
        }
